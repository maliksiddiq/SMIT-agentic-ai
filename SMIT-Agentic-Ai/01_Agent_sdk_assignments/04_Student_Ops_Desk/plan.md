...
- **Model:** `gemini-3.6-flash`, configured at the agent level.
...
The base agent carries the shared model configuration (`gemini-3.6-flash`).  
...
- **Model settings:** temperature 0.2 — deliberately low, because assignment deadline and policy answers must be deterministic and factual. The model itself (gemini-3.6-flash) is inherited from the base agent, not re-declared.
...
- **Model settings:** temperature 0.7 — deliberately higher, because career guidance benefits from a warmer, more varied tone. The model itself (gemini-3.6-flash) is inherited from the base agent, not re-declared.
